/*
 * $Id: gtxvt.c 9725 2012-10-02 10:56:51Z andijahja $
 */
                                  ...
                                  int left, int top, int right, int bottom )
{
   ...
   {
      if( buf->rInvalid.x1 > left )
         buf->rInvalid.x1 = left;
      if( buf->rInvalid.y1 > top )
         buf->rInvalid.y1 = top;
      if( buf->rInvalid.x2 < right )
         buf->rInvalid.x2 = right;
      if( buf->rInvalid.y2 < bottom )
         buf->rInvalid.y2 = bottom;
   }
             ...
             select( streamUpdate[ 0 ] + 1, &updateSet, NULL, NULL, &timeout ) )
      {
         ...
         switch( appMsg )
         {
            ...
            case XVT_ICM_UPDATE:
            {
               ...
               if( added.x1 < rUpdate.x1 )
                  rUpdate.x1 = added.x1;
               if( added.y1 < rUpdate.y1 )
                  rUpdate.y1 = added.y1;
               if( added.x2 > rUpdate.x2 )
                  rUpdate.x2 = added.x2;
               if( added.y2 > rUpdate.y2 )
                  rUpdate.y2 = added.y2;
            }
            break;
...
void HB_GT_FUNC( gt_Replicate( USHORT usRow, USHORT usCol, BYTE byAttr, BYTE byChar, ULONG ulLen ) )
{
   ...
   if( ulLen > XVT_CHAR_BUFFER ) // Avoid allocating memory if possible
   {
...
void HB_GT_FUNC( gt_Scroll( USHORT usTop, USHORT usLeft, USHORT usBottom, USHORT usRight, BYTE byAttr, SHORT iRows, SHORT iCols ) )
{
   ...
   if( iLength > XVT_CHAR_BUFFER ) // Avoid allocating memory if possible
   {
