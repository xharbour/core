/*
 * $Id: dbcmd53.c 9766 2012-10-16 20:57:27Z andijahja $
 */
...
HB_FUNC( DBINFO )
{
   ...
   if( pArea )
   {
      ...
      if( pIndex )
      {
         ...
         SELF_INFO( pArea, ( USHORT ) hb_itemGetNI( pIndex ), pInfo );
...
HB_FUNC( DBORDERINFO )
{
   ...
   if( pArea )
   {
      ...
      if( pType )
      {
         ...
         SELF_ORDINFO( pArea, ( USHORT ) hb_itemGetNI( pType ), &pOrderInfo );
         hb_itemReturnRelease( pOrderInfo.itmResult );
      }
      else
...
HB_FUNC( DBFIELDINFO )
{
   ...
   if( pArea )
   {
          ...
          uiIndex > 0 && uiIndex <= uiFields )
      {
         ...
         SELF_FIELDINFO( pArea, uiIndex, ( USHORT) hb_itemGetNI( pType ), pInfo );
         hb_itemReturnRelease( pInfo );
      }
      else
         hb_errRT_DBCMD( EG_ARG, EDBCMD_DBCMDBADPARAMETER, NULL, HB_ERR_FUNCNAME );
...
HB_FUNC( DBRECORDINFO )
{
   ...
   if( pArea )
   {
      ...
      if( pType )
      {
         ...
         SELF_RECINFO( pArea, pRecNo, ( USHORT ) hb_itemGetNI( pType ), pInfo );
         hb_itemReturnRelease( pInfo );
      }
      else
   ...
   if( pArea )
   {
          ...
          uiIndex <= uiFields )
      {
         hb_retl( SELF_GETVALUEFILE( pArea, uiIndex, hb_parc( 2 ),
                                     ( USHORT ) hb_itemGetNI( pMode ) ) == HB_SUCCESS );
      }
      else
