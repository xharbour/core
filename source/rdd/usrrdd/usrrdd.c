/*
 * $Id: usrrdd.c 9766 2012-10-16 20:57:27Z andijahja $
 */
...
static HB_ERRCODE hb_usrEvalRddFunc( PHB_ITEM pMethods, USHORT uiMethod, USHORT uiRddID )
{
...
static HB_ERRCODE hb_usrEvalAreaFunc( PHB_ITEM pMethods, USHORT uiMethod, AREAP pArea )
{
   if( hb_usrPushMethod( pMethods, uiMethod ) )
   {
...
static BOOL hb_usrItemToFieldInfo( PHB_ITEM pItem, LPDBFIELDINFO pFieldInfo )
{
   if( pItem && hb_arrayLen( pItem ) == UR_FI_SIZE )
   {
      ...
      pFieldInfo->uiType         = ( USHORT ) hb_arrayGetNI( pItem, UR_FI_TYPE );
      pFieldInfo->uiTypeExtended = ( USHORT ) hb_arrayGetNI( pItem, UR_FI_TYPEEXT );
      pFieldInfo->uiLen          = ( USHORT ) hb_arrayGetNI( pItem, UR_FI_LEN );
      pFieldInfo->uiDec          = ( USHORT ) hb_arrayGetNI( pItem, UR_FI_DEC );
      pFieldInfo->uiFlags        = ( USHORT ) hb_arrayGetNI( pItem, UR_FI_FLAGS );
      pFieldInfo->uiStep         = ( USHORT ) hb_arrayGetNI( pItem, UR_FI_STEP );
      return TRUE;
   }
   return FALSE;
}
...
static BOOL hb_usrItemToOpenInfo( PHB_ITEM pItem, LPDBOPENINFO pOpenInfo )
{
   ...
   {
      pOpenInfo->uiArea       = ( USHORT ) hb_arrayGetNI( pItem, UR_OI_AREA );
...
static BOOL hb_usrItemToLockInfo( PHB_ITEM pItem, LPDBLOCKINFO pLockInfo )
{
   if( pItem && hb_arrayLen( pItem ) == UR_LI_SIZE )
   {
      ...
      pLockInfo->uiMethod  = ( USHORT ) hb_arrayGetNI( pItem, UR_LI_METHOD );
      pLockInfo->fResult   = ( USHORT ) hb_arrayGetL( pItem, UR_LI_RESULT );
      return TRUE;
   }
   return FALSE;
}
...
static BOOL hb_usrItemToTransInfo( PHB_ITEM pItem, LPDBTRANSINFO pTransInfo )
{
   ...
   {
      USHORT   uiItemCount = ( USHORT ) hb_arrayGetNI( pItem, UR_TI_ITEMCOUNT ), uiCount;
      PHB_ITEM pItems      = hb_arrayGetItemPtr( pItem, UR_TI_ITEMS ), pItm;
                                 ...
                                 &pTransInfo->dbsci ) )
      {
         ...
         pTransInfo->uiFlags     = ( USHORT ) hb_arrayGetNI( pItem, UR_TI_FLAGS );
         ...
         if( uiItemCount )
         {
            ...
            {
               pItm                 = hb_arrayGetItemPtr( pItems, uiCount );
               pTransItem->uiSource = ( USHORT ) hb_arrayGetNI( pItm, UR_TITEM_SOURCE );
               pTransItem->uiDest   = ( USHORT ) hb_arrayGetNI( pItm, UR_TITEM_DESTIN );
            }
         }
         else
         {
...
static BOOL hb_usrItemToSortInfo( PHB_ITEM pItem, LPDBSORTINFO pSortInfo )
{
   ...
   {
      USHORT   uiItemCount = ( USHORT ) hb_arrayGetNI( pItem, UR_SRI_ITEMCOUNT ), uiCount;
      PHB_ITEM pItems      = hb_arrayGetItemPtr( pItem, UR_SRI_ITEMS ), pItm;
                                 ...
                                 &pSortInfo->dbtri ) )
      {
         ...
         if( uiItemCount )
         {
            ...
            {
               pItm                 = hb_arrayGetItemPtr( pItems, uiCount );
               pSortItem->uiField   = ( USHORT ) hb_arrayGetNI( pItm, UR_SITEM_FIELD );
               pSortItem->uiFlags   = ( USHORT ) hb_arrayGetNI( pItm, UR_SITEM_FLAGS );
            }
         }
         else
         {
...
static HB_ERRCODE hb_usrStructSize( AREAP pArea, USHORT * puiSize )
{
   HB_ERRCODE errCode;
...
static HB_ERRCODE hb_usrFieldCount( AREAP pArea, USHORT * puiFields )
{
   LONG lOffset;
   ...
   *puiFields = ( USHORT ) hb_itemGetNI( hb_stackItemFromBase( lOffset ) );
   hb_stackPop();
...
static HB_ERRCODE hb_usrGetValue( AREAP pArea, USHORT uiIndex, PHB_ITEM pItem )
{
...
static HB_ERRCODE hb_usrPutValue( AREAP pArea, USHORT uiIndex, PHB_ITEM pItem )
{
...
static HB_ERRCODE hb_usrGetVarLen( AREAP pArea, USHORT uiIndex, ULONG * pulLength )
{
   LONG lOffset;
...
static HB_ERRCODE hb_usrRecInfo( AREAP pArea, PHB_ITEM pRecID, USHORT uiInfoType, PHB_ITEM pInfo )
{
...
static HB_ERRCODE hb_usrFieldInfo( AREAP pArea, USHORT uiIndex, USHORT uiInfoType, PHB_ITEM pInfo )
{
...
static HB_ERRCODE hb_usrSetFieldExtent( AREAP pArea, USHORT uiFieldExtent )
{
...
static HB_ERRCODE hb_usrInfo( AREAP pArea, USHORT uiInfoType, PHB_ITEM pInfo )
{
...
static HB_ERRCODE hb_usrRelArea( AREAP pArea, USHORT uiRelNo, USHORT * puiRelArea )
{
   ...
   *puiRelArea = ( USHORT ) hb_itemGetNI( hb_stackItemFromBase( lOffset ) );
   hb_stackPop();
...
static HB_ERRCODE hb_usrRelText( AREAP pArea, USHORT uiRelNo, PHB_ITEM pExpr )
{
...
static HB_ERRCODE hb_usrOrderInfo( AREAP pArea, USHORT uiIndex, LPDBORDERINFO pOrderInfo )
{
   PHB_ITEM pItem, pResult;
...
static HB_ERRCODE hb_usrRawLock( AREAP pArea, USHORT uiAction, ULONG ulRecNo )
{
...
static HB_ERRCODE hb_usrGetValueFile( AREAP pArea, USHORT uiIndex, const char * szFile, USHORT uiMode )
{
...
static HB_ERRCODE hb_usrPutValueFile( AREAP pArea, USHORT uiIndex, const char * szFile, USHORT uiMode )
{
...
static HB_ERRCODE hb_usrDrop( LPRDDNODE pRDD, PHB_ITEM pTable, PHB_ITEM pIndex, ULONG ulConnect )
{
...
static HB_ERRCODE hb_usrExists( LPRDDNODE pRDD, PHB_ITEM pTable, PHB_ITEM pIndex, ULONG ulConnect )
{
...
{
   USHORT uiRddID = ( USHORT ) hb_parni( 1 );
...
HB_FUNC_UR_SUPER( FIELDNAME )
{
   ...
   if( pArea )
   {
      ...
      hb_retni( SUPER_FIELDNAME( pArea, ( USHORT ) hb_parni( 2 ), szName ) );
      hb_storc( szName, 3 );
      hb_xfree( szName );
   }
}
...
HB_FUNC_UR_SUPER( GETVALUE )
{
   ...
   if( pArea )
      hb_retni( SUPER_GETVALUE( pArea, ( USHORT ) hb_parni( 2 ), hb_param( 3, HB_IT_ANY ) ) );
}
...
HB_FUNC_UR_SUPER( PUTVALUE )
{
   ...
   if( pArea )
      hb_retni( SUPER_PUTVALUE( pArea, ( USHORT ) hb_parni( 2 ), hb_param( 3, HB_IT_ANY ) ) );
}
...
HB_FUNC_UR_SUPER( GETVARLEN )
{
   ...
   if( pArea )
   {
      ...
      hb_retni( SUPER_GETVARLEN( pArea, ( USHORT ) hb_parni( 2 ), &ulLength ) );
...
HB_FUNC_UR_SUPER( RECINFO )
{
   ...
   if( pArea )
      hb_retni( SUPER_RECINFO( pArea, hb_param( 2, HB_IT_ANY ), ( USHORT ) hb_parni( 3 ),
...
HB_FUNC_UR_SUPER( FIELDINFO )
{
   ...
   if( pArea )
      hb_retni( SUPER_FIELDINFO( pArea, ( USHORT ) hb_parni( 2 ), ( USHORT ) hb_parni( 3 ),
                                 hb_param( 4, HB_IT_ANY ) ) );
}
...
HB_FUNC_UR_SUPER( SETFIELDEXTENT )
{
   ...
   if( pArea )
      hb_retni( SUPER_SETFIELDEXTENT( pArea, ( USHORT ) hb_parni( 2 ) ) );
}
...
HB_FUNC_UR_SUPER( INFO )
{
   ...
   if( pArea )
      hb_retni( SUPER_INFO( pArea, ( USHORT ) hb_parni( 2 ), hb_param( 3, HB_IT_ANY ) ) );
}
...
HB_FUNC_UR_SUPER( RELAREA )
{
   ...
   if( pArea )
   {
      ...
      hb_retni( SUPER_RELAREA( pArea, ( USHORT ) hb_parni( 2 ), &uiRelArea ) );
...
HB_FUNC_UR_SUPER( RELTEXT )
{
   ...
   if( pArea )
      hb_retni( SUPER_RELTEXT( pArea, ( USHORT ) hb_parni( 2 ), hb_param( 3, HB_IT_ANY ) ) );
}
...
HB_FUNC_UR_SUPER( ORDINFO )
{
   ...
   if( pArea )
   {
      ...
      if( hb_usrItemToOrderInfo( pItem, &dbOrderInfo ) )
      {
         hb_retni( SUPER_ORDINFO( pArea, ( USHORT ) hb_parni( 2 ), &dbOrderInfo ) );
         hb_arraySet( pItem, UR_ORI_RESULT, dbOrderInfo.itmResult );
      }
      else
      {
...
HB_FUNC_UR_SUPER( RAWLOCK )
{
   ...
   if( pArea )
      hb_retni( SUPER_RAWLOCK( pArea, ( USHORT ) hb_parni( 2 ), hb_parnl( 3 ) ) );
}
...
HB_FUNC_UR_SUPER( GETVALUEFILE )
{
   ...
   if( pArea )
      hb_retni( SUPER_GETVALUEFILE( pArea, ( USHORT ) hb_parni( 2 ), hb_parc( 3 ),
                                    ( USHORT ) hb_parni( 4 ) ) );
}
...
HB_FUNC_UR_SUPER( PUTVALUEFILE )
{
   ...
   if( pArea )
      hb_retni( SUPER_PUTVALUEFILE( pArea, ( USHORT ) hb_parni( 2 ), hb_parc( 3 ),
                                    ( USHORT ) hb_parni( 4 ) ) );
}
...
HB_FUNC_UR_SUPER( RDDINFO )
{
   ...
   if( pRDD )
      hb_retni( SUPER_RDDINFO( pRDD, ( USHORT ) hb_parni( 2 ), hb_parnl( 3 ),
                               hb_param( 4, HB_IT_ANY ) ) );
}
