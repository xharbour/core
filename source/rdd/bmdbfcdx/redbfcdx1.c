/*
 * $Id: redbfcdx1.c 9723 2012-10-02 09:18:08Z andijahja $
 */
 ...
 *    RE_Turbo( lOnOff )
 *    RE_DbGetFilterArray() => aFilterRec
 *    RE_DbSetFilterArray( aFilterRec )
 *    RE_DbSetFilterArrayAdd( aFilterRec )
 *    RE_DbSetFilterArrayDel( aFilterRec )
 *    hb_cdxAppend
...
static ULONG hb_cdxIndexGetAvailPage( LPCDXINDEX pIndex, BOOL bHeader )
{
   ...
   if( pIndex->freePage != 0 && pIndex->freePage != CDX_DUMMYNODE && ! bHeader )
   {
      ...
      {
         if( hb_fileNetReadAt( pFile, byBuf, 4, ulPos ) != 4 )
   ...
   else
   {
      ...
      if( pIndex->nextAvail != CDX_DUMMYNODE )
         ...
         ulPos = ( ULONG ) hb_fileNetSize( pFile );
      ...
      if( bHeader )
      {
         ...
         do
         {
            if( hb_fileNetWriteAt( pFile, byPageBuf, CDX_PAGELEN, fOffset ) != CDX_PAGELEN )
...
static void hb_cdxIndexFlushAvailPage( LPCDXINDEX pIndex )
{
   ...
   while( pLst && pLst->fStat )
   {
      ...
      if( hb_fileNetWriteAt( pIndex->pFile, byPageBuf, CDX_PAGELEN, ulPos ) != CDX_PAGELEN )
      {
         hb_errInternal( EDBF_WRITE, "Write in index page failed.", NULL, NULL );
      }
      pIndex->fChanged  = TRUE;
                                  ...
                                  USHORT uiSize )
{
   ...
   if( hb_fileNetWriteAt( pIndex->pFile, pBuffer, uiSize, ulPos ) != ( ULONG ) uiSize )
      hb_errInternal( EDBF_WRITE, "Write in index page failed.", NULL, NULL );
   pIndex->fChanged = TRUE;
}
                                 ...
                                 USHORT uiSize )
{
   ...
   if( hb_fileNetReadAt( pIndex->pFile, pBuffer, uiSize, ulPos ) != ( ULONG ) uiSize )
...
static void hb_cdxIndexCheckVersion( LPCDXINDEX pIndex )
{
   ...
   if( hb_fileNetReadAt( pIndex->pFile, byBuf, 8, 0x04 ) != 8 )
   {
      if( pIndex->lockWrite > 0 && hb_fileNetSize( pIndex->pFile ) == 0 )
         memset( byBuf, 0, 8 );
      else
...
static BOOL hb_cdxIndexLockRead( LPCDXINDEX pIndex )
{
   ...
   {
      ret = hb_dbfnetLockIdxFile( pIndex->pFile, pIndex->pArea->dbfarea.bLockType,
...
static BOOL hb_cdxIndexUnLockRead( LPCDXINDEX pIndex )
{
   ...
   else
   {
          ...
          ! HB_DIRTYREAD( &pIndex->pArea->dbfarea ) )
      {
         ...
         pIndex->RdLck = FALSE;
#endif
         if( ! hb_dbfnetLockIdxFile( pIndex->pFile, pIndex->pArea->dbfarea.bLockType, FL_UNLOCK, &pIndex->ulLockPos ) )
         {
            hb_errInternal( 9108, "hb_cdxIndexUnLockRead: unlock error.", NULL, NULL );
         }
      }
...
static BOOL hb_cdxIndexUnLockWrite( LPCDXINDEX pIndex )
{
   ...
   if( pIndex->pArea->dbfarea.fShared && pIndex->fShared )
   {
      if( pIndex->fChanged )
      {
         ...
         if( hb_fileNetWriteAt( pIndex->pFile, byBuf, 8, 0x04 ) != 8 )
         {
   ...
   else
   {
      if( pIndex->ulVersion != pIndex->freePage )
      {
         BYTE byBuf[ 4 ];
         HB_PUT_LE_UINT32( &byBuf[ 0 ], pIndex->freePage );
         if( hb_fileNetWriteAt( pIndex->pFile, byBuf, 4, 0x04 ) != 4 )
         {
...
static void hb_cdxTagLoad( LPCDXTAG pTag )
{
   ...
   if( pTag->RootBlock == 0 || pTag->RootBlock % CDX_PAGELEN != 0 ||
       ( HB_FOFFSET ) pTag->RootBlock >= hb_fileNetSize( pTag->pIndex->pFile ) ||
...
static void hb_cdxIndexReindex( LPCDXINDEX pIndex )
{
   ...
   pIndex->nextAvail = 0;
   pIndex->freePage  = 0;
   hb_fileNetTruncAt( pIndex->pFile, 0 );
   pIndex->fChanged  = TRUE;
...
static void hb_cdxIndexFree( LPCDXINDEX pIndex )
{
   ...
   if( pIndex->pFile )
   {
      hb_fileNetClose( pIndex->pFile );
      if( pIndex->fDelete )
      {
         hb_fileNetDelete( ( pIndex->szRealName ?
                             pIndex->szRealName : pIndex->szFileName ), pIndex->uiRemote );
      }
   }
...
HB_FUNC( RE_DBSETFILTERARRAYADD )
{
   HB_THREAD_STUB
   AREAP pArea       = ( AREAP ) hb_rddGetCurrentWorkAreaPointer();
   PHB_ITEM pArray   = hb_param( 1, HB_IT_ARRAY );
...
HB_FUNC( RE_DBSETFILTERARRAYDEL )
{
   HB_THREAD_STUB
   AREAP pArea       = ( AREAP ) hb_rddGetCurrentWorkAreaPointer();
   PHB_ITEM pArray   = hb_param( 1, HB_IT_ARRAY );
...
static HB_ERRCODE hb_cdxOpen( CDXAREAP pArea, LPDBOPENINFO pOpenInfo )
{
   ...
   if( DBFAREA_DATA( &pArea->dbfarea )->fStrictStruct ? pArea->dbfarea.fHasTags : hb_setGetAutOpen() )
   {
      ...
      if( hb_FileNetExists( szFileName, NULL ) ||
 ...
 */
   ...
   do
   {
      pFile = hb_fileNetExtOpen( szFileName, NULL, uiFlags |
      ...
      {
         if( hb_fileNetSize( pFile ) <= ( HB_FOFFSET ) sizeof( CDXTAGHEADER ) )
         {
...
static HB_ERRCODE hb_cdxOrderCreate( CDXAREAP pArea, LPDBORDERCREATEINFO pOrderInfo )
{
   ...
   if( ! fOpenedIndex )
   {
      ...
      do
      {
         if( fTemporary || pOrderInfo->uiRemote == 1 )
         {
            pFile    = hb_fileNetCreateTemp( NULL, NULL, fTemporary ? FC_TEMPORARY : FC_NORMAL, szTempFile );
            fNewFile = TRUE;
         }
         else
         {
            pFile = hb_fileNetExtOpen( szFileName, NULL, FO_READWRITE |
         ...
         else
         {
            bRetry = FALSE;
            if( ! fNewFile )
               fNewFile = ( hb_fileNetSize( pFile ) == 0 );
         }
      }
      while( bRetry );
      ...
      if( pFile )
      {
         ...
         pIndex->uiRemote     = pOrderInfo->uiRemote;
         if( fTemporary )
...
static HB_ERRCODE hb_cdxSetFilter( CDXAREAP pArea, LPDBFILTERINFO pFilterInfo )
{
   ...
   if( pArea->dbfarea.area.dbfi.fOptimized )
   {
      ...
      PHB_FILE pDataFile = pArea->dbfarea.pDataFile;

      /* Get dbf file to temp buffer */
      pArea->dbfarea.pDataFile                                    = hb_fileNetGetFileToTemp( pDataFile, pszFileName );
      ...
      {
         char pszIndexFileName[ HB_PATH_MAX ];
      ...
      /* restore dbf */
      hb_fileNetClose( pArea->dbfarea.pDataFile );
      hb_fsDelete( pszFileName );
      pArea->dbfarea.pDataFile = pDataFile;
...
static void hb_cdxSortWritePage( LPCDXSORTINFO pSort )
{
   ...
   if( ! pSort->pTempFile )
   {
      ...
      pSort->pTempFile = hb_fileNetCreateTemp( NULL, NULL, FC_TEMPORARY, szName );
      if( ! pSort->pTempFile )
      {
   ...
   pSort->pSwapPage[ pSort->ulCurPage ].nOffset = hb_fileNetSeekLarge( pSort->pTempFile, 0, FS_END );
   if( hb_fileNetWriteLarge( pSort->pTempFile, pSort->pKeyPool, ulSize ) != ulSize )
   {
                                  ...
                                  BYTE ** pKeyVal, ULONG * pulRec )
{
   ...
   if( pSort->pSwapPage[ ulPage ].ulKeyBuf == 0 )
   {
      ...
      if( hb_fileNetSeekLarge( pSort->pTempFile, pSort->pSwapPage[ ulPage ].nOffset, SEEK_SET ) != pSort->pSwapPage[ ulPage ].nOffset ||
          hb_fileNetReadLarge( pSort->pTempFile, pSort->pSwapPage[ ulPage ].pKeyPool, ulSize ) != ulSize )
      {
...
static void hb_cdxSortFree( LPCDXSORTINFO pSort )
{
   ...
   if( pSort->pTempFile )
   {
      uiRemote = hb_fileNetRemote( pSort->pTempFile );
      hb_fileNetClose( pSort->pTempFile );
   }
   else
      uiRemote = 0;
   ...
   {
      hb_fileNetDelete( ( pSort->szTempFileName ), uiRemote );
      ...
      while( ! pArea->dbfarea.area.fEof )
      {
         if( fDirectRead )
         {
            ...
            if( iRecBuff == 0 || iRecBuff >= iRecBufSize )
            {
               ...
               hb_fileNetReadAt( pArea->dbfarea.pDataFile, pSort->pRecBuff, pArea->dbfarea.uiRecordLen * iRec,
...
HB_FUNC( REDBFCDX_GETFUNCTABLE )
{
   ...
   HB_TRACE( HB_TR_DEBUG, ( "REDBFCDX_GETFUNCTABLE(%p, %p)", uiCount, pTable ) );
   ...
   if( pTable )
   {
      ...
      errCode = hb_rddInherit( pTable, &cdxTable, &cdxSuper, ( const char * ) "REDBFFPT" );
      if( errCode != HB_SUCCESS )
         errCode = hb_rddInherit( pTable, &cdxTable, &cdxSuper, ( const char * ) "REDBFDBT" );
      if( errCode != HB_SUCCESS )
         errCode = hb_rddInherit( pTable, &cdxTable, &cdxSuper, ( const char * ) "DBFNET" );
      hb_retni( errCode );
   }
   else
      hb_retni( HB_FAILURE );
...
static void hb_redbfcdxRddInit( void * cargo )
{
   HB_SYMBOL_UNUSED( cargo );

   if( hb_rddRegister( "DBFNET", RDT_FULL ) <= 1 )
   {
      hb_rddRegister( "REDBFFPT", RDT_FULL );
      if( hb_rddRegister( "REDBFCDX", RDT_FULL ) <= 1 )
      {
         return;
      }
   }
...
HB_INIT_SYMBOLS_BEGIN( redbfcdx1__InitSymbols )
{
   "REDBFCDX", { HB_FS_PUBLIC | HB_FS_LOCAL }, { HB_FUNCNAME( REDBFCDX ) }, NULL
},
{ "REDBFCDX_GETFUNCTABLE", { HB_FS_PUBLIC | HB_FS_LOCAL }, { HB_FUNCNAME( REDBFCDX_GETFUNCTABLE ) }, NULL }
HB_INIT_SYMBOLS_END( redbfcdx1__InitSymbols )

HB_CALL_ON_STARTUP_BEGIN( _hb_redbfcdx_rdd_init_ )
hb_vmAtInit( hb_redbfcdxRddInit, NULL );
HB_CALL_ON_STARTUP_END( _hb_redbfcdx_rdd_init_ )

#if defined( HB_PRAGMA_STARTUP )
   #pragma startup redbfcdx1__InitSymbols
   #pragma startup _hb_redbfcdx_rdd_init_
#elif defined( HB_DATASEG_STARTUP )
   #define HB_DATASEG_BODY HB_DATASEG_FUNC( redbfcdx1__InitSymbols ) \
   HB_DATASEG_FUNC( _hb_redbfcdx_rdd_init_ )
   #include "hbiniseg.h"
#endif
