/*
 * $Id: dbfcdx1.c 9723 2012-10-02 09:18:08Z andijahja $
 */
...
static void hb_cdxTagSetScope( LPCDXTAG pTag, USHORT nScope, PHB_ITEM pItem )
{
   ...
   if( hb_cdxItemTypeCmp( ( BYTE ) pTag->uiType ) == hb_cdxItemTypeCmp( ( BYTE ) hb_cdxItemType( pScopeVal ) ) )
   {
      PHB_ITEM *  pScope;
      LPCDXKEY *  pScopeKey;
      ULONG       ulRec;
...
static void hb_cdxIndexFree( LPCDXINDEX pIndex )
{
   ...
   if( pIndex->pFile )
   {
      ...
      if( pIndex->fDelete )
         hb_fileDelete(  pIndex->szRealName ? ( const char * ) pIndex->szRealName : ( const char * ) pIndex->szFileName, ( USHORT ) 0 );
   }
