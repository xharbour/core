/*
 * $Id: dbffpt1.c 9766 2012-10-16 20:57:27Z andijahja $
 */
      ...
      if( ulBlock != 0 )
      {
                              ...
                              pArea->bMemoType == DB_MEMO_FPT ) )
         {
            ...
            if( pArea->bMemoType == DB_MEMO_DBT )
            {
               ...
               do
               {
                  ulLen    = ( ULONG ) hb_fileReadAt( pArea->pMemoFile, pBlock, DBT_DEFBLOCKSIZE, fOffset );
                  fOffset  += ulLen;
                  if( ulLen <= 0 )
                     break;
                  u        = 0;
                                       ...
                                       HB_FOFFSET size )
{
   ...
   if( size )
   {
      ...
      do
      {
         ulRead = ( ULONG ) hb_fileReadAt( pSrc, pBuffer, ( ULONG )
...
static HB_ERRCODE hb_fptReadRawBlock( FPTAREAP pArea, BYTE * bBuffer, HB_FHANDLE hFile, ULONG ulBlock, ULONG ulSize )
{
   HB_ERRCODE errCode = HB_SUCCESS;
   ...
   if( hFile != FS_ERROR )
   {
      errCode = hb_fptCopyToRawFile( pArea->pMemoFile, FPT_BLOCK_OFFSET( ulBlock ), hFile, ulSize );
   }
   else
   {
                                       ...
                                       USHORT uiMode )
{
   ...
   if( hFile != FS_ERROR )
      return hb_fptCopyToRawFile( pArea->pMemoFile, FPT_BLOCK_OFFSET( ulBlock ) + 4, hFile, ulSize );
...
static HB_ERRCODE hb_fptWriteMemo( FPTAREAP pArea, ULONG ulBlock, ULONG ulSize,
                                   const BYTE * bBufPtr, HB_FHANDLE hFile,
   ...
   if( bWrite )
   {
      ...
      if( errCode == HB_SUCCESS && ulLen > 0 )
      {
         ...
         if( hFile != FS_ERROR )
         {
            ...
            do
            {
               ulRead = ( ULONG ) hb_fsReadLarge( hFile, bBuffer, HB_MIN( ulBufSize, ulLen - ulWritten ) );
               if( ulRead <= 0 )
                  errCode = EDBF_READ;
               else if( hb_fileWriteAt( pArea->pMemoFile, bBuffer, ulRead, fOffset ) != ulRead )
                  errCode = EDBF_WRITE;
               else
               {
                  ulWritten   += ulRead;
...
static HB_ERRCODE hb_fptGetVarField( FPTAREAP pArea, USHORT uiIndex, PHB_ITEM pItem, HB_FHANDLE hFile )
{
   LPFIELD     pField;
   HB_ERRCODE  errCode;
   BYTE *      pFieldBuf;
      ...
      else if( pField->uiLen >= 6 )
      {
         ...
         if( uiType <= HB_VF_CHAR ) /* 64000 max string size */
         {
            ...
            else
            {
               ...
               errCode = hb_fptReadRawBlock( pArea, ( BYTE * ) pBuf, FS_ERROR, ulBlock, ulSize );
            }
         ...
         else if( uiType == HB_VF_ARRAY ) /* MEMO only as SMT ARRAY */
         {
            if( hFile != FS_ERROR )
               ...
               errCode = hb_fptReadSMTBlock( pArea, pItem, ulBlock, 0,
                                             ( BYTE ) pArea->area.lpFields[ uiIndex - 1 ].uiFlags );
         }
...
static HB_ERRCODE hb_fptGetVarFile( FPTAREAP pArea, ULONG ulBlock, const char * szFile, USHORT uiMode )
{
   ...
   else
   {
      ...
      errCode = hb_fptGetMemo( pArea, 0, NULL, hFile, ulBlock, 0, 0 );
      hb_fsClose( hFile );
   }
   ...
   if( errCode != HB_SUCCESS )
   {
      if( errCode != HB_FAILURE )
      {
         hb_memoErrorRT( pArea, 0, errCode,
                         ...
                         errCode == EDBF_WRITE ? ( char * ) szFile :
 ...
 */
      ...
      {
         if( pArea->fTemporary )
            pArea->pMemoFile = hb_fileCreateTempEx( szFileName, NULL, NULL, NULL, FC_TEMPORARY );
         else
