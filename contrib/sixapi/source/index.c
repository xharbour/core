/*
 * $Id: index.c 9753 2012-10-14 10:51:37Z andijahja $
 */
...
HB_FUNC( SX_INDEXTAG )
{
   ...
   cpFileName = ISCHAR( 1 ) ? ( PBYTE ) hb_parc( 1 ) : ( PBYTE ) 0;
   cpTagName  = ( PBYTE ) hb_parc( 2 );
   iIndexType = ISNUM( 4 ) ? ( WORD ) hb_parni( 4 ) : ( WORD ) 0;

   if( iIndexType > 2 )
      iIndexType = 0;

   bDescend      = ISLOG( 5 ) ? hb_parl( 5 ) : FALSE;
   cpCondition   = ISCHAR( 6 ) ? ( PBYTE ) hb_parc( 6 ) : ( PBYTE ) 0;
...
HB_FUNC( SX_KEYADD )
{
   ...
   cpTagName = ISCHAR( 1 ) ? ( PBYTE ) hb_parc( 1 ) : ( PBYTE ) 0;
...
HB_FUNC( SX_KEYDROP )
{
   ...
   cpTagname = ISCHAR( 1 ) ? ( PBYTE ) hb_parc( 1 ) : ( PBYTE ) NULL;
...
HB_FUNC( SX_TAGINFO )
{
   ...
   {
      nOldOrd    = sx_IndexOrd();
      nTagCount  = ( USHORT ) sx_SysProp( SDE_SP_GETINDEXCOUNT, ( PVOID ) NULL );
   ...
   hb_itemCopy( HB_STACK_RETURN(), pTagInfo );
