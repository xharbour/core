/*
 * $Id: use.c 9753 2012-10-14 10:51:37Z andijahja $
 */
                     ...
                     int iRDEType )
{
   ...
   OpenInfo.iRDEType      = ( USHORT ) iRDEType;
   OpenInfo.iMode         = ( USHORT ) iOpenMode;
   OpenInfo.cRDD          = cRDD[ iRDEType - 1 ];
   OpenInfo.iCommitLevel  = ( USHORT ) sx_GetCommitLevel( ( WORD ) iOpenedArea );
   OpenInfo.iRecSize      = ( USHORT ) sx_RecSize();
...
HB_FUNC( SX_USE )
{
   if( ISCHAR( 1 ) )
   {
      ...
      if( hb_fsFileExists( ( const char * ) szDBFFileName ) )
      {
         ...
         /* RDE Type Passed? */
         iRDEType = ISCHAR( 4 ) ? _sx_CheckRDD( hb_parc( 4 ) ) : i_sxApi_RDD_Default;
   ...
   if( Opened_DBF_Property )
   {
      if( ISCHAR( 1 ) || ISNUM( 1 ) )
      {
         ...
         if( iLen > 1 )
         {
            ...
            for( ui = 0; ui < (USHORT) HB_ARRAY_LEN( Opened_DBF_Property );
                 ui++ )
            {
               BOOL           bFound     = FALSE;
      ...
      else
      {
         if( ISLOG( 2 ) && hb_parl( 2 ) )
         {
            ...
            HB_ARRAY_CLONE( pInfo, Opened_DBF_Property );

            for( i = 0; i < HB_ARRAY_LEN( pInfo ); i++ )
            {
...
BOOL _sx_Used( void )
{
   return Opened_DBF_Property ? HB_ARRAY_LEN( Opened_DBF_Property ) > 0 : FALSE;
}

HB_FUNC( SX_USED )
{
   hb_retl( Opened_DBF_Property ? HB_ARRAY_LEN( Opened_DBF_Property ) > 0 : FALSE );
}
