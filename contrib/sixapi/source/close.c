/*
 * $Id: close.c 9576 2012-07-17 16:41:57Z andijahja $
 */
...
void _sx_DelOpenInfo( const char * szAlias )
{
   ...
   {
      USHORT uiSize;
      for( uiSize = 0; uiSize < ( USHORT ) HB_ARRAY_LEN( Opened_DBF_Property ); uiSize++ )
      {
         ...
         if( cAliasInfo )
         {
            if( strcmp( cAliasInfo, szAlias ) == 0 )
            {
               ...
               hb_arraySize( Opened_DBF_Property, HB_ARRAY_LEN( Opened_DBF_Property ) - 1 );
               break;
            }
         }
      }
