/*
 * $Id: put.c 9753 2012-10-14 10:51:37Z andijahja $
 */
...
HB_FUNC( SX_PUTVALUEEX )  // ( aValues, nArea )
{
   ...
   if( ISARRAY( 1 ) )
   {
      ...
      USHORT   uiLen   = ( USHORT ) hb_arrayLen( pValue );
      USHORT   uiSize;
      WORD     uVarType;
      PBYTE    cFieldName;
      ...
      for( uiSize = 0; uiSize < uiLen; uiSize++ )
      {
         ...
         switch( uVarType )
         {
            ...
            case HB_IT_LOGICAL:
            {
               BOOL bValue = hb_arrayGetL( pValue, uiSize + 1 );
...
/*
   ...
   Logical field values should be passed as either zero (FALSE) or -1 (TRUE).
