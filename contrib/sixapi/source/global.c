/*
 * $Id: global.c 9753 2012-10-14 10:51:37Z andijahja $
 */
...
static BOOL          bCenturyIsOn  = FALSE;
static char *        aFormat[]     =
{
...
static BOOL          lDeletedIsOn  = FALSE;
static int           iBaseYear     = 0;
static BOOL          bSetExactIsOn = FALSE;
static BOOL          lSoftSeekIsOn;
...
HB_FUNC( SX_SETSOFTSEEK )
{
   ...
   else if( ISCHAR( 1 ) )
   {
      ...
      lSoftSeekIsOn = ( strcmp( _sx_upper( szTmp ), "ON" ) == 0 ) ? TRUE : FALSE;
...
/*
   ...
   VOID FAR PASCAL sx_SetExact (USHORT uiOnOff);
...
HB_FUNC( SX_SETEXACT )
{
   ...
   else if( ISCHAR( 1 ) )
   {
      ...
      bSetExactIsOn = ( strcmp( _sx_upper( szTmp ), "ON" ) == 0 ) ? TRUE : FALSE;
...
/*
   ...
   USHORT FAR PASCAL sx_SetEpoch (USHORT uiBaseYear);
...
/*
   ...
   VOID FAR PASCAL sx_SetDeleted(USHORT uiDeleted);

   Description
   Makes deleted records either transparent or visible to record positioning
   functions.

   Setting sx_SetDeleted to TRUE incurs certain performance penalties. Instead
   of using sx_SetDeleted TRUE, consider creating conditional index files with a
...
HB_FUNC( SX_SETDELETED )
{
   ...
   else if( ISCHAR( 1 ) )
   {
      ...
      lDeletedIsOn = ( strcmp( _sx_upper( szTmp ), "ON" ) == 0 ) ? TRUE : FALSE;
...
/*
   ...
   VOID FAR PASCAL sx_SetDateFormat (USHORT uiDateType);
...
HB_FUNC( SX_SETCENTURY )
{
   ...
   else if( ISCHAR( 1 ) )
   {
      ...
      bCenturyIsOn = ( strcmp( _sx_upper( szTmp ), "ON" ) == 0 ) ? TRUE : FALSE;
...
/*
   ...
   VOID FAR PASCAL sx_SetStringType (USHORT uiStringType);
...
/*
   ...
   VOID FAR PASCAL sx_SetTranslate (USHORT uiOnOff);
   ...
   first call sx_SetTranslate (FALSE) and afterwards call sx_SetTranslate (TRUE).
 */
...
HB_FUNC( SX_SETGETTRIMMED )
{
   ...
   else if( ISCHAR( 1 ) )
   {
      ...
      bSetTrimmedON = ( strcmp( _sx_upper( szTmp ), "ON" ) == 0 ) ? TRUE : FALSE;
