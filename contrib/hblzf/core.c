...
static ULONG hb_lzf_compressbound( ULONG nLen )
{
   ULONG nBuffSize = ( ULONG ) ( nLen * 1.04 + 1 );
...
HB_FUNC( HB_LZF_DECOMPRESS )
{
   ...
   if( pArg )
   {
      ...
      if( in_len )
      {
         ...
         {
            buffer_size = ( ISNUM( 2 ) && hb_parnl( 2 ) >= 0 ) ?
                          hb_parnl( 2 ) : HB_LZF_DEFAULT_BUFFSIZE;
