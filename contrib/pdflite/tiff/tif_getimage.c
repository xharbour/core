...
TIFFRGBAImageBegin(TIFFRGBAImage* img, TIFF* tif, int stop, char emsg[1024])
{
	...
	if (!pickTileContigCase(tif, img)) {
		sprintf(emsg, "Sorry, can not handle image");
		return 0;
	}
    } else {
	img->get = TIFFIsTiled(tif) ? gtTileSeparate : gtStripSeparate;
	if (!pickTileSeparateCase(tif, img)) {
		sprintf(emsg, "Sorry, can not handle image");
		return 0;
	}
    }
...
TIFFRGBAImageGet(TIFFRGBAImage* img, uint32* raster, uint32 w, uint32 h)
{
    ...
    if (img->get == NULL) {
	_TIFFError(tif, TIFFFileName(img->tif), "No \"get\" routine setup");
	return (0);
    }
    if (img->put.any == NULL) {
	_TIFFError(tif, TIFFFileName(img->tif),
			  ...
			  int orientation, int stop)
{
	...
	_TIFFError(tif, TIFFFileName(tif), emsg);
	ok = 0;
    }
    return (ok);
}
...
gtTileContig(TIFFRGBAImage* img, uint32* raster, uint32 w, uint32 h)
{
    ...
    if (buf == 0) {
	_TIFFError(tif, TIFFFileName(tif), "No space for tile buffer");
	return (0);
    }
...
gtStripContig(TIFFRGBAImage* img, uint32* raster, uint32 w, uint32 h)
{
    ...
    if (buf == 0) {
	_TIFFError(tif, TIFFFileName(tif), "No space for strip buffer");
	return (0);
    }
...
makebwmap(TIFF* tif, TIFFRGBAImage* img)
{
    ...
    if (img->BWmap == NULL) {
	_TIFFError(tif,TIFFFileName(img->tif),"No space for B&W mapping table");
	return (0);
    }
...
buildMap(TIFF* tif, TIFFRGBAImage* img)
{
	...
	if (checkcmap(img) == 16)
	    ...
	    _TIFFWarning(tif, TIFFFileName(img->tif),"Assuming 8-bit colormap");
	/*
...

{
    ...
    if( TIFFIsTiled( tif ) )
    {
        _TIFFError(tif, TIFFFileName(tif),
