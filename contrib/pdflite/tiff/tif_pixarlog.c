/* PDFlib GmbH cvsid:
 * $Id: tif_pixarlog.c 9644 2012-08-11 05:55:52Z andijahja $ */
/*
 ...
 */
    ...
    ToLinear8=(unsigned char *)_TIFFmalloc(*sizeof(unsigned char));
...
"PixarLog compression can't handle bits depth/data format combination\
(depth: %d)", 
			td->td_bitspersample);
		return (0);
	}
...
PixarLogDecode(TIFF* tif, tidata_t op, tsize_t occ, tsample_t s)
{
	...
	do {
		...
		if (state != Z_OK) {
			_TIFFError(tif, module, "%s: zlib error: %s",
			    tif->tif_name, sp->stream.msg);
			return (0);
		}
...
PixarLogEncode(TIFF* tif, tidata_t bp, tsize_t cc, tsample_t s)
{
	...
	do {
		if (deflate(&sp->stream, Z_NO_FLUSH) != Z_OK) {
			_TIFFError(tif, module, "%s: Encoder error: %s",
			    tif->tif_name, sp->stream.msg);
			return (0);
		}
		if (sp->stream.avail_out == 0) {
...
TIFFInitPixarLog(TIFF* tif, int scheme)
{
	...
	TIFFMergeFieldInfo(tif, pixarlogFieldInfo, N(pixarlogFieldInfo));
	...
	return (1);
bad:
	_TIFFError(tif, "TIFFInitPixarLog",
