/* PDFlib GmbH cvsid:
 * $Id: tif_dir.c 9734 2012-10-09 19:54:17Z andijahja $ */
...
void _TIFFsetByteArray(TIFF* tif, void** vpp, void* vp, uint32 n)
    { setByteArray(tif, vpp, vp, n, 1); }
void _TIFFsetString(TIFF* tif, char** cpp, char* cp)
    { setByteArray(tif, (void**) cpp, (void*) cp, strlen(cp)+1, 1); }
void _TIFFsetNString(TIFF* tif, char** cpp, char* cp, uint32 n)
    { setByteArray(tif, (void**) cpp, (void*) cp, n, 1); }
void _TIFFsetShortArray(TIFF* tif, uint16** wpp, uint16* wp, uint32 n)
    { setByteArray(tif, (void**) wpp, (void*) wp, n, sizeof (uint16)); }
void _TIFFsetLongArray(TIFF* tif, uint32** lpp, uint32* lp, uint32 n)
    { setByteArray(tif, (void**) lpp, (void*) lp, n, sizeof (uint32)); }
void _TIFFsetFloatArray(TIFF* tif, float** fpp, float* fp, uint32 n)
    { setByteArray(tif, (void**) fpp, (void*) fp, n, sizeof (float)); }
void _TIFFsetDoubleArray(TIFF* tif, double** dpp, double* dp, uint32 n)
 ...
 */
static int
setExtraSamples(TIFF* tif, TIFFDirectory* td, va_list ap, uint32* v)
{
/* PDFlib GmbH: bug #2424, fix backported from libtiff 4.0.0alpha */
...
_TIFFVSetField(TIFF* tif, ttag_t tag, va_list ap)
{
	...
	case TIFFTAG_SUBIFD:
		if ((tif->tif_flags & TIFF_INSUBIFD) == 0) {
			...
			_TIFFError(tif, module,"%s: Sorry, cannot nest SubIFDs",
				  tif->tif_name);
			status = 0;
		}
		break;
	...
	case TIFFTAG_XMLPACKET:
	    ...
	    if (tv_size == 0) {
		    status = 0;
		    _TIFFError(tif, module, "%s: Bad field type %d for \"%s\"",
		...
		_TIFFsetString(tif, (char **)&tv->value, va_arg(ap, char *));
	    else {
		...
		if (fip->field_passcount
			...
			case TIFF_UNDEFINED:
			    {
				uint8 lv = (uint8)va_arg(ap, int);
				_TIFFmemcpy(tv->value, &lv, tv_size*tv->count);
			    }
			    break;
			case TIFF_SBYTE:
			    {
				int8 lv = (int8)va_arg(ap, int);
				_TIFFmemcpy(tv->value, &lv, tv_size*tv->count);
			    }
			    break;
			case TIFF_SHORT:
			    {
				uint16 lv = (uint16)va_arg(ap, int);
				_TIFFmemcpy(tv->value, &lv, tv_size*tv->count);
			    }
			    break;
			case TIFF_SSHORT:
			    {
				int16 lv = (int16)va_arg(ap, int);
				_TIFFmemcpy(tv->value, &lv, tv_size*tv->count);
			    }
			    break;
			case TIFF_LONG:
			case TIFF_IFD:
			    {
				uint32 lv = va_arg(ap, uint32);
				_TIFFmemcpy(tv->value, &lv, tv_size*tv->count);
			    }
			    break;
			case TIFF_SLONG:
			    {
				int32 lv = va_arg(ap, int32);
				_TIFFmemcpy(tv->value, &lv, tv_size*tv->count);
			    }
			    break;
			case TIFF_RATIONAL:
			...
			case TIFF_FLOAT:
			    {
				float lv = (float)va_arg(ap, double);
				_TIFFmemcpy(tv->value, &lv, tv_size*tv->count);
			    }
			    break;
			case TIFF_DOUBLE:
			    {
				double lv = va_arg(ap, double);
				_TIFFmemcpy(tv->value, &lv, tv_size*tv->count);
			    }
			    break;
			default:
	...
	return (status);
badvalue:
	_TIFFError(tif, module, "%s: Bad value %d for \"%s\"",
		  tif->tif_name, v, TIFFFieldWithTag(tif, tag)->field_name);
	va_end(ap);
	return (0);
badvalue32:
	_TIFFError(tif, module, "%s: Bad value %ld for \"%s\"",
		   tif->tif_name, v32, TIFFFieldWithTag(tif, tag)->field_name);
	va_end(ap);
	return (0);
badvaluedbl:
	_TIFFError(tif, module, "%s: Bad value %f for \"%s\"",
...
OkToChangeTag(TIFF* tif, ttag_t tag)
{
	const TIFFFieldInfo* fip = TIFFFindFieldInfo(tif, tag, TIFF_ANY);
	if (!fip) {			/* unknown tag */
		_TIFFError(tif, "TIFFSetField", "%s: Unknown %stag %u",
...
{
	register TIFFDirectory* ltd = &tif->tif_dir;

	_TIFFSetupFieldInfo(tif);
	_TIFFmemset(ltd, 0, sizeof (*ltd));
	ltd->td_fillorder = FILLORDER_MSB2LSB;
	ltd->td_bitspersample = 1;
	ltd->td_threshholding = THRESHHOLD_BILEVEL;
	ltd->td_orientation = ORIENTATION_TOPLEFT;
	ltd->td_samplesperpixel = 1;
	ltd->td_rowsperstrip = (uint32) -1;
	ltd->td_tilewidth = 0;
	ltd->td_tilelength = 0;
	ltd->td_tiledepth = 1;
	ltd->td_stripbytecountsorted = 1; /* Our own arrays always sorted. */
	ltd->td_resolutionunit = RESUNIT_INCH;
	ltd->td_sampleformat = SAMPLEFORMAT_UINT;
	ltd->td_imagedepth = 1;
	ltd->td_ycbcrsubsampling[0] = 2;
	ltd->td_ycbcrsubsampling[1] = 2;
	ltd->td_ycbcrpositioning = YCBCRPOSITION_CENTERED;
	ltd->td_inkset = INKSET_CMYK;
	ltd->td_ninks = 4;
...
TIFFAdvanceDirectory(TIFF* tif, uint32* nextdir, toff_t* off)
{
    ...
    if (isMapped(tif))
    {
        ...
        {
            _TIFFError(tif, module, "%s: Error fetching directory count",
                      tif->tif_name);
            return (0);
        }
        ...
        {
            _TIFFError(tif, module, "%s: Error fetching directory link",
                      tif->tif_name);
            return (0);
        }
...
TIFFUnlinkDirectory(TIFF* tif, tdir_t dirn)
{
	...
	if (!WriteOK(tif, &nextdir, sizeof (uint32))) {
		_TIFFError(tif, module, "Error writing directory link");
		return (0);
	}
	/*
