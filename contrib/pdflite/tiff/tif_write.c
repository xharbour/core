/* PDFlib GmbH cvsid:
 * $Id: tif_write.c 9644 2012-08-11 05:55:52Z andijahja $ */
...
TIFFWriteEncodedTile(TIFF* tif, ttile_t tile, tdata_t data, tsize_t cc)
{
	...
	if (tile >= td->td_nstrips) {
		_TIFFError(tif, module, "%s: Tile %lu out of range, max %lu",
	 ...
	 */
	tif->tif_row = (tile % TIFFhowmany(td->td_imagelength,
		td->td_tilelength))
		* td->td_tilelength;
...
TIFFWriteCheck(TIFF* tif, int tiles, const char* module)
{
	if (tif->tif_mode == O_RDONLY) {
		_TIFFError(tif, module, "%s: File not open for writing",
		    tif->tif_name);
		return (0);
	}
	...
	if (tif->tif_dir.td_stripoffset == NULL && !TIFFSetupStrips(tif)) {
		...
		_TIFFError(tif, module, "%s: No space for %s arrays",
...
TIFFWriteBufferSetup(TIFF* tif, tdata_t bp, tsize_t size)
{
	...
	if (bp == NULL) {
		bp = _TIFFmalloc(size);
		if (bp == NULL) {
			_TIFFError(tif,module,"%s: No space for output buffer",
			    tif->tif_name);
			return (0);
		}
		tif->tif_flags |= TIFF_MYBUFFER;
...
TIFFGrowStrips(TIFF* tif, int delta, const char* module)
{
	...
	if (new_stripoffset == NULL || new_stripbytecount == NULL) {
		...
		td->td_nstrips = 0;
		_TIFFError(tif, module, "%s: No space to expand strip arrays",
			  tif->tif_name);
		return (0);
	}
...
TIFFAppendToStrip(TIFF* tif, tstrip_t strip, tidata_t data, tsize_t cc)
{
	...
	if (!WriteOK(tif, data, cc)) {
		_TIFFError(tif, module, "%s: Write error at scanline %lu",
