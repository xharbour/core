/* PDFlib GmbH cvsid:
 * $Id: tif_dirwrite.c 9644 2012-08-11 05:55:52Z andijahja $ */
...
_TIFFWriteDirectory(TIFF* tif, int done)
{
	...
	if (!WriteOK(tif, &dircount, sizeof (dircount))) {
		_TIFFError(tif, tif->tif_name, "Error writing directory count");
		goto bad;
	}
	...
	if (!WriteOK(tif, &diroff, sizeof (diroff))) {
		_TIFFError(tif, tif->tif_name, "Error writing directory link");
		goto bad;
	}
	if (done) {
...
TIFFRewriteDirectory( TIFF *tif )
{
    ...
    */
    ...
    if (tif->tif_header.tiff_diroff == tif->tif_diroff) 
    {
        ...
        {
            _TIFFError(tif, tif->tif_name, "Error updating TIFF header");
            return (0);
        }
    }
    else
	...
	do {
		...
		if (!SeekOK(tif, nextdir) ||
			...
			_TIFFError(tif,module,"Error fetching directory count");
			return (0);
		}
		if (tif->tif_flags & TIFF_SWAB)
			TIFFSwabShort(&dircount);
		...
		if (!ReadOK(tif, &nextdir, sizeof (nextdir))) {
			_TIFFError(tif, module,"Error fetching directory link");
			return (0);
		}
		if (tif->tif_flags & TIFF_SWAB)
