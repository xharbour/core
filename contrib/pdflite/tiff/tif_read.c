/* PDFlib GmbH cvsid:
 * $Id: tif_read.c 9644 2012-08-11 05:55:52Z andijahja $ */
...
TIFFSeek(TIFF* tif, uint32 row, tsample_t sample)
{
	...
	if (row >= td->td_imagelength) {	/* out of range */
		_TIFFError(tif, tif->tif_name, "%lu: Row out of range, max %lu",
...
TIFFReadEncodedStrip(TIFF* tif, tstrip_t strip, tdata_t buf, tsize_t size)
{
	...
	if (strip >= td->td_nstrips) {
		_TIFFError(tif,tif->tif_name,"%ld: Strip out of range, max %ld",
...
TIFFReadRawStrip(TIFF* tif, tstrip_t strip, tdata_t buf, tsize_t size)
{
	...
	if (!TIFFCheckRead(tif, 0))
		return ((tsize_t) -1);
	if (strip >= td->td_nstrips) {
		_TIFFError(tif,tif->tif_name,"%lu: Strip out of range, max %lu",
...
TIFFReadEncodedTile(TIFF* tif, ttile_t tile, tdata_t buf, tsize_t size)
{
	...
	if (tile >= td->td_nstrips) {
		_TIFFError(tif, tif->tif_name,"%ld: Tile out of range, max %ld",
...
TIFFReadRawTile(TIFF* tif, ttile_t tile, tdata_t buf, tsize_t size)
{
	...
	if (!TIFFCheckRead(tif, 1))
		return ((tsize_t) -1);
	if (tile >= td->td_nstrips) {
		_TIFFError(tif, tif->tif_name,"%lu: Tile out of range, max %lu",
...
TIFFReadBufferSetup(TIFF* tif, tdata_t bp, tsize_t size)
{
	...
	if (bp) {
		...
		tif->tif_rawdata =
		    (tidata_t) _TIFFmalloc(tif->tif_rawdatasize);
		tif->tif_flags |= TIFF_MYBUFFER;
	}
...
TIFFCheckRead(TIFF* tif, int tiles)
{
	if (tif->tif_mode == O_WRONLY) {
		_TIFFError(tif, tif->tif_name, "File not open for reading");
		return (0);
	}
	if (tiles ^ isTiled(tif)) {
		_TIFFError(tif, tif->tif_name, tiles ?
