/* PDFlib GmbH cvsid:
 * $Id: tif_compress.c 9644 2012-08-11 05:55:52Z andijahja $ */
...
TIFFRegisterCODEC(uint16 scheme, const char* name, TIFFInitMethod init)
{
	...
	if (cd != NULL) {
		...
		_TIFFError(tif, "TIFFRegisterCODEC",
...
TIFFUnRegisterCODEC(TIFFCodec* c)
{
	...
	_TIFFError(tif, "TIFFUnRegisterCODEC",
...
TIFFCodec*
TIFFGetConfiguredCODECs(TIFF *tif)
{
	int		i = 1;
        codec_t		*cd;
        const TIFFCodec	*c;
	TIFFCodec	*codecs = NULL, *new_codecs;
        ...
        for (c = _TIFFBuiltinCODECS; c->name; c++) {
			...
			codecs = new_codecs;
			_TIFFmemcpy(codecs + i - 1, (const tdata_t)c,
					sizeof(TIFFCodec));
			i++;
		}
	}
