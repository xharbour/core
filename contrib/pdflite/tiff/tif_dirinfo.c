/* PDFlib GmbH cvsid:
 * $Id: tif_dirinfo.c 9734 2012-10-09 19:54:17Z andijahja $ */
...
_TIFFFindFieldInfoByName(TIFF* tif, const char *field_name, TIFFDataType dt)
{
	...
	/* NB: use sorted search (e.g. binary search) */
	if(dt != TIFF_ANY) {
            TIFFFieldInfo key = {(ttag_t)0, (short)0, (short)0, (TIFFDataType)0,
		(unsigned short)0, (unsigned char)0, (unsigned char)0,
		(char *)0};
            key.field_name = (char *)field_name;
            key.field_type = dt;
            return((const TIFFFieldInfo *) pdc_lfind(&key,
						 tif->tif_fieldinfo, 
						 &tif->tif_nfields,
						 sizeof(TIFFFieldInfo),
						 tagNameCompare));
...
_TIFFFieldWithName(TIFF* tif, const char *field_name)
{
	...
	if (!fip) {
		_TIFFError(tif, "TIFFFieldWithName",
