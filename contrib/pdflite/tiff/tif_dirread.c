/* PDFlib GmbH cvsid
 * :$Id: tif_dirread.c 9644 2012-08-11 05:55:52Z andijahja $
 */
...
"%s: invalid TIFF directory; tags are not sorted in ascending order",
	...
	if (!td->td_nstrips) {
		_TIFFError(tif, module, "%s: cannot handle zero number of %s",
	...
	for (dp = dir, n = dircount; n > 0; n--, dp++) {
		...
		case TIFFTAG_TRANSFERFUNCTION:
			...
			v *= sizeof(uint16);
			cp = (char *)_TIFFCheckMalloc(tif,dp->tdir_count,
			    sizeof(uint16), "to read \"TransferFunction\" tag");
			if (cp != NULL) {
...
#define	BYTECOUNTLOOKSBAD \
    ( (td->td_stripbytecount[0] == 0 && td->td_stripoffset[0] != 0) || \
      (td->td_compression == COMPRESSION_NONE && \
       td->td_stripbytecount[0] > TIFFGetFileSize(tif) \
       - td->td_stripoffset[0]) || \
	...
	if (!tif->tif_scanlinesize) {
		_TIFFError(tif, module, "%s: cannot handle zero scanline size",
			  tif->tif_name);
		return (0);
	}
...
TIFFFetchData(TIFF* tif, TIFFDirEntry* dir, char* cp)
{
	...
	return (cc);
bad:
	_TIFFError(tif, tif->tif_name, "Error fetching data for field \"%s\"",
	    TIFFFieldWithTag(tif, dir->tdir_tag)->field_name);
	return ((tsize_t) 0);
}
        ...
        if (tif->tif_header.tiff_magic == TIFF_BIGENDIAN) {
            if (dir->tdir_type == TIFF_SBYTE)
                switch (dir->tdir_count) {
                    case 4: v[3] = (uint8) (dir->tdir_offset & 0xff);
                    case 3: v[2] = (uint8) ((dir->tdir_offset >> 8) & 0xff);
                    case 2: v[1] = (uint8) ((dir->tdir_offset >> 16) & 0xff);
                    case 1: v[0] = (uint8) (dir->tdir_offset >> 24);
                }
            else
                switch (dir->tdir_count) {
                    case 4: v[3] = (uint8) (dir->tdir_offset & 0xff);
            ...
            if (dir->tdir_type == TIFF_SBYTE)
                switch (dir->tdir_count) {
                    case 4: v[3] = (uint8) (dir->tdir_offset >> 24);
                    case 3: v[2] = (uint8) ((dir->tdir_offset >> 16) & 0xff);
                    case 2: v[1] = (uint8) ((dir->tdir_offset >> 8) & 0xff);
                    case 1: v[0] = (uint8) (dir->tdir_offset & 0xff);
                }
            else
                switch (dir->tdir_count) {
                    case 4: v[3] = (uint8) (dir->tdir_offset >> 24);
...
TIFFFetchRefBlackWhite(TIFF* tif, TIFFDirEntry* dir)
{
	 ...
	 */
	cp = (char *)_TIFFCheckMalloc(tif, dir->tdir_count, sizeof (uint32),
			mesg);
