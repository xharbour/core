/* PDFlib GmbH cvsid:
 * $Id: tif_luv.c 9734 2012-10-09 19:54:17Z andijahja $ */
/*
...
LogL16InitState(TIFF* tif)
{
	...
	if (multiply(sp->tbuflen, sizeof (int16)) == 0 ||
	    (sp->tbuf = (tidata_t*)
			_TIFFmalloc(sp->tbuflen * sizeof (int16))) == NULL) {
...
LogLuvInitState(TIFF* tif)
{
	...
	if (multiply(sp->tbuflen, sizeof (uint32)) == 0 ||
	    (sp->tbuf = (tidata_t*)
			_TIFFmalloc(sp->tbuflen * sizeof (uint32))) == NULL) {
		_TIFFError(tif, module,
			"%s: No space for SGILog translation buffer",
		    tif->tif_name);
		return (0);
...
TIFFInitSGILog(TIFF* tif, int scheme)
{
	...
	return (1);
bad:
	_TIFFError(tif, module, "%s: No space for LogLuv state block",
		tif->tif_name);
	return (0);
}
