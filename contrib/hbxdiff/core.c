/*
 * $Id: core.c 9816 2012-11-11 18:35:45Z andijahja $
 */
...
static PHB_ITEM hb_mmf_itemPut( PHB_ITEM pItem, void * pMemAddr, int iType )
{
   ...
   pStructHolder = ( PHB_MMF_HOLDER ) hb_gcAlloc( sizeof( HB_MMF_HOLDER ),
                                                     hb_mmf_destructor );
...
static int xdlt_outb( void * priv, mmbuffer_t * mb, int nbuf )
{
   ...
   if( pCallback )
   {
      ...
      if ( HB_IS_POINTER( pCallback ) )
      {
         hb_vmPushSymbol( ( PHB_SYMB ) hb_itemGetPtr( pCallback ) );
         hb_vmPushNil();
      }
      else if ( HB_IS_BLOCK( pCallback ) )
      {
         hb_vmRequestReenter();
         hb_vmPushEvalSym();
         hb_vmPush( pCallback );
...
HB_FUNC( XDL_DIFF )
{
   ...
   if( phb_mmf1 && phb_mmf1->mmf && phb_mmf2 && phb_mmf2->mmf )
   {
      ...
      else if( HB_ISBLOCK( 5 ) || HB_ISPOINTER( 5 ) )
      {
         PHB_ITEM pCallback = hb_param( 5, HB_IT_BLOCK | HB_IT_POINTER );
...
HB_FUNC( XDL_BDIFF )
{
   ...
   if( phb_mmf1 && phb_mmf1->mmf && phb_mmf2 && phb_mmf2->mmf )
   {
      ...
      else if( HB_ISBLOCK( 4 ) || HB_ISPOINTER( 4 ) )
      {
         PHB_ITEM pCallback = hb_param( 4, HB_IT_BLOCK | HB_IT_POINTER );
...
HB_FUNC( XDL_RABDIFF )
{
   ...
   if( phb_mmf1 && phb_mmf1->mmf && phb_mmf2 && phb_mmf2->mmf )
   {
      ...
      else if( HB_ISBLOCK( 3 ) || HB_ISPOINTER( 3 ) )
      {
         PHB_ITEM pCallback = hb_param( 3, HB_IT_BLOCK | HB_IT_POINTER );
