...
#xcommand T( <(title)>, <(subject)> ) => ;
          ...
          OutStd( sprintf( <title> + ": t: [%s] ", magic_buffer( hMagic, <subject> ) ) ) ;;
          magic_setflags( hMagic, MAGIC_MIME_TYPE ) ;;
          OutStd( sprintf( "m: [%s]", magic_buffer( hMagic, <subject> ) ) + hb_osnewline() )
...
PROCEDURE Main()
   ...
   IF Empty( hMagic ) .OR. magic_load( hMagic ) != 0
      OutStd( "magic_open()/magic_load() failed" + hb_osnewline() )
      QUIT
   ENDIF
   ...
   OutStd( "hb_Magic_Simple(): t: [" + hb_Magic_Simple( hb_argv( 0 ), MAGIC_NONE ) + "] " + ;
            "m: [" + hb_Magic_Simple( hb_argv( 0 ), MAGIC_MIME_TYPE ) + "]" + hb_osnewline() )
