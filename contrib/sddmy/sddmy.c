/*
 * $Id: sddmy.c 9773 2012-10-19 08:29:51Z andijahja $
 */
...
static HB_ERRCODE mysqlConnect( SQLDDCONNECTION * pConnection, PHB_ITEM pItem )
{
   ...
   {
      hb_rddsqlSetError( (HB_ERRCODE) mysql_errno( pMySql ), mysql_error( pMySql ), NULL, NULL, 0 );
      mysql_close( pMySql );
      return HB_FAILURE;
   }
...
static HB_ERRCODE mysqlExecute( SQLDDCONNECTION * pConnection, PHB_ITEM pItem )
{
   ...
   {
      hb_rddsqlSetError( (HB_ERRCODE) mysql_errno( pMySql ), mysql_error( pMySql ), hb_itemGetCPtr( pItem ), NULL, 0 );
      return HB_FAILURE;
   }
   ...
   else
   {
      ...
      {
         ulAffectedRows = (ULONG) mysql_affected_rows( pMySql );
...
/*         if ( pFieldInfo.uiType == HB_IT_DOUBLE || pFieldInfo.uiType == HB_IT_INTEGER )
   ...
   pArea->ulRecCount = ( ULONG ) mysql_num_rows( pSDDData->pResult );
