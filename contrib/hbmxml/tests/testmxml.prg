...
PROCEDURE _APPMain( cFileArg )
   ...
   IF Empty( cFileArg )
      OutErr( "Usage: textmxml filename.xml" + hb_osnewline() )
      ErrorLevel( 1 )
      QUIT
   ENDIF
   ...
   IF Empty( hTree )
      OutErr( "ERROR: No parent node in basic test!" + hb_osnewline() )
      QUIT
   ENDIF
   ...
   IF nNum != MXML_ELEMENT
      IF nNum < MXML_ELEMENT .OR. nNum > MXML_TEXT
         OutErr( sprintf( "ERROR: Parent has type %s (%d), expected MXML_ELEMENT!",  ;
                               "UNKNOWN", nNum ) + hb_osnewline() )
      ELSE
         OutErr( sprintf( "ERROR: Parent has type %s (%d), expected MXML_ELEMENT!",  ;
                               s_aTypes[ nNum + 1 ], nNum ) + hb_osnewline() )
      ENDIF
   ...
   IF cStr != "element"
      OutErr( sprintf( "ERROR: Parent value is '%s', expected 'element'", cStr ) + hb_osnewline() )
   ...
   IF Empty( hNode := mxmlGetFirstChild( hTree ) )
      OutErr( "ERROR: No first child in basic test!" + hb_osnewline() )
   ...
   IF nNum != MXML_INTEGER
      IF nNum < MXML_ELEMENT .OR. nNum > MXML_TEXT
         OutErr( sprintf( "ERROR: First child has type %s (%d), expected MXML_TEXT!",  ;
                               "UNKNOWN", nNum ) + hb_osnewline() )
      ELSE
         OutErr( sprintf( "ERROR: First child has type %s (%d), expected MXML_TEXT!",  ;
                               s_aTypes[ nNum + 1 ], nNum ) + hb_osnewline() )
   ...
   IF nNum != 123
      OutErr( sprintf( "ERROR: First child value is %d, expected 123!", nNum ) + hb_osnewline() )
   ...
   IF Empty( hNode )
      OutErr( "ERROR: No second child node in basic test!" + hb_osnewline() )
   ...
   IF nNum != MXML_OPAQUE
      IF nNum < MXML_ELEMENT .OR. nNum > MXML_TEXT
         OutErr( sprintf( "ERROR: Second child has type %s (%d), expected MXML_OPAQUE!",  ;
                               "UNKNOWN", nNum ) + hb_osnewline() )
      ELSE
         OutErr( sprintf( "ERROR: Second child has type %s (%d), expected MXML_OPAQUE!",  ;
                               s_aTypes[ nNum + 1 ], nNum ) + hb_osnewline() )
   ...
   hNode := mxmlGetNextSibling( hNode )
   IF Empty( hNode )
      OutErr( "ERROR: No third child node in basic test!" + hb_osnewline() )
   ...
   IF nNum != MXML_REAL
      IF nNum < MXML_ELEMENT .OR. nNum > MXML_TEXT
         OutErr( sprintf( "ERROR: Third child has type %s (%d), expected MXML_REAL!",  ;
                               "UNKNOWN", nNum ) + hb_osnewline() )
      ELSE
         OutErr( sprintf( "ERROR: Third child has type %s (%d), expected MXML_REAL!",  ;
                               s_aTypes[ nNum + 1 ], nNum ) + hb_osnewline() )
   ...
   IF nNum != 123.4
      OutErr( sprintf( "ERROR: Third child value is %f, expected 123.4!", nNum ) + hb_osnewline() )
   ...
   hNode := mxmlGetNextSibling( hNode )
   IF Empty( hNode )
      OutErr( "ERROR: No fourth child node in basic test!" + hb_osnewline() )
   ...
   IF nNum != MXML_TEXT
      IF nNum < MXML_ELEMENT .OR. nNum > MXML_TEXT
         OutErr( sprintf( "ERROR: Fourth child has type %s (%d), expected MXML_TEXT!",  ;
                               "UNKNOWN", nNum ) + hb_osnewline() )
      ELSE
         OutErr( sprintf( "ERROR: Fourth child has type %s (%d), expected MXML_TEXT!",  ;
                               s_aTypes[ nNum + 1 ], nNum ) + hb_osnewline() )
   ...
   IF nNum != 1 .OR. Empty( cStr ) .OR. cStr != "text"
      OutErr( sprintf( "ERROR: Fourth child value is %d, '%s', expected 1, 'text'!",    ;
                            nNum, cStr ) + hb_osnewline() )
   ...
   FOR i := 0 TO 3
      IF Empty( hNode := mxmlGetNextSibling( hNode ) )
         OutErr( sprintf( "ERROR: No group #%d child node in basic test!", i ) + hb_osnewline() )
      ...
      IF ( ( nNum := mxmlGetType( hNode ) ) != MXML_ELEMENT )
         IF nNum < MXML_ELEMENT .OR. nNum > MXML_TEXT
            OutErr( sprintf( "ERROR: Group child #%d has type %s (%d), expected MXML_ELEMENT!",  ;
                                  i + 1, "UNKNOWN", nNum ) + hb_osnewline() )
         ELSE
            OutErr( sprintf( "ERROR: Group child #%d has type %s (%d), expected MXML_ELEMENT!",  ;
                                  s_aTypes[ nNum + 1 ], nNum ) + hb_osnewline() )
         ENDIF
   ...
   IF Empty( hNode )
      OutErr( "ERROR: Unable to find value for '*/two'." + hb_osnewline() )

      mxmlDelete( hTree )
      ErrorLevel( 1 )
      QUIT
   ELSEIF mxmlGetType( hNode ) != MXML_OPAQUE .OR. mxmlGetOpaque( hNode ) != "value"
      OutErr( "ERROR: Bad value for '*/two'." + hb_osnewline() )
   ...
   IF Empty( hNode )
      OutErr( "ERROR: Unable to find value for 'foo/*/two'." + hb_osnewline() )

      mxmlDelete( hTree )
      ErrorLevel( 1 )
      QUIT
   ELSEIF mxmlGetType( hNode ) != MXML_OPAQUE .OR. mxmlGetOpaque( hNode ) != "value"
      OutErr( "ERROR: Bad value for 'foo/*/two'." + hb_osnewline() )
   ...
   IF Empty( hNode )
      OutErr( "ERROR: Unable to find value for 'foo/bar/one/two'." + hb_osnewline() )

      mxmlDelete( hTree )
      ErrorLevel( 1 )
      QUIT
   ELSEIF mxmlGetType( hNode ) != MXML_OPAQUE .OR. mxmlGetOpaque( hNode ) != "value"
      OutErr( "ERROR: Bad value for 'foo/bar/one/two'." + hb_osnewline() )
   ...
   IF Empty( hInd )
      OutErr( "ERROR: Unable to create index of all nodes!" + hb_osnewline() )
   ...
   IF ( nNum := mxmlIndexGetCount( hInd ) ) != 10
      OutErr( sprintf( "ERROR: Index of all nodes contains %d nodes; expected 10!", nNum ) + hb_osnewline() )
   ...
   IF Empty( mxmlIndexFind( hInd, "group" ) )
      OutErr( "ERROR: mxmlIndexFind for 'group' failed!" + hb_osnewline() )
   ...
   IF Empty( hInd )
      OutErr( "ERROR: Unable to create index of groups!" + hb_osnewline() )
   ...
   IF ( nNum := mxmlIndexGetCount( hInd ) ) != 4
      OutErr( sprintf( "ERROR: Index of groups contains %d nodes; expected 4!", nNum ) + hb_osnewline() )
   ...
   IF Empty( mxmlIndexEnum( hInd ) )
      OutErr( "ERROR: mxmlIndexEnum failed!" + hb_osnewline() )
   ...
   IF Empty( hInd )
      OutErr( "ERROR: Unable to create index of type attributes!" + hb_osnewline() )
   ...
   IF ( nNum := mxmlIndexGetCount( hInd ) ) != 3
      OutErr( sprintf( "ERROR: Index of type attributes contains %d nodes; expected 3!", nNum ) + hb_osnewline() )
   ...
   IF Empty( mxmlIndexFind( hInd,, "string" ) )
      OutErr( "ERROR: mxmlIndexFind for 'string' failed!" + hb_osnewline() )
   ...
   IF Empty( hInd )
      OutErr( "ERROR: Unable to create index of elements and attributes!" + hb_osnewline() )
   ...
   FOR i := 0 TO 8
      ...
      ELSE
         OutErr( sprintf( "ERROR: Child pointer prematurely NULL on child #%d", i ) + hb_osnewline() )
   ...
   IF ! Empty( mxmlGetFirstChild( hTree ) )
      OutErr( "ERROR: Child pointer not NULL after deleting all children!" + hb_osnewline() )
   ...
   IF ! Empty( mxmlGetLastChild( hTree ) )
      OutErr( "ERROR: Last child pointer not NULL after deleting all children!" + hb_osnewline() )
   ...
   IF Empty( hTree )
      OutErr( "Unable to read XML file!" + hb_osnewline() )
      ErrorLevel( 1 )
      QUIT
   ENDIF
   ...
   IF cFileArg == "test.xml"
      ...
      IF Empty( hNode := mxmlFindElement( hTree, hTree, "choice",,, MXML_DESCEND ) )
         OutErr( "Unable to find first <choice> element in XML tree!" + hb_osnewline() )
      ...
      IF Empty( mxmlFindElement( hNode, hTree, "choice",,, MXML_NO_DESCEND ) )
         OutErr( "Unable to find second <choice> element in XML tree!" + hb_osnewline() )
   ...
   IF cFileArg == "test.xml"
      IF s_aSAXEventCounts[ MXML_SAX_CDATA ] != 1
         OutErr( sprintf( "MXML_SAX_CDATA seen %d times, expected 1 times!",                     ;
                               s_aSAXEventCounts[ MXML_SAX_CDATA ] ) + hb_osnewline() )
         ErrorLevel( 1 )
         QUIT
      ENDIF

      IF s_aSAXEventCounts[ MXML_SAX_COMMENT ] != 1
         OutErr( sprintf( "MXML_SAX_COMMENT seen %d times, expected 1 times!",                   ;
                               s_aSAXEventCounts[ MXML_SAX_COMMENT ] ) + hb_osnewline() )
         ErrorLevel( 1 )
         QUIT
      ENDIF

      IF s_aSAXEventCounts[ MXML_SAX_DATA ] != 60
         OutErr( sprintf( "MXML_SAX_DATA seen %d times, expected 60 times!",                     ;
                               s_aSAXEventCounts[ MXML_SAX_DATA ] ) + hb_osnewline() )
         ErrorLevel( 1 )
         QUIT
      ENDIF

      IF s_aSAXEventCounts[ MXML_SAX_DIRECTIVE ] != 1
         OutErr( sprintf( "MXML_SAX_DIRECTIVE seen %d times, expected 1 times!",                 ;
                               s_aSAXEventCounts[ MXML_SAX_DIRECTIVE ] ) + hb_osnewline() )
         ErrorLevel( 1 )
         QUIT
      ENDIF

      IF s_aSAXEventCounts[ MXML_SAX_ELEMENT_CLOSE ] != 20
         OutErr( sprintf( "MXML_SAX_ELEMENT_CLOSE seen %d times, expected 20 times!",            ;
                               s_aSAXEventCounts[ MXML_SAX_ELEMENT_CLOSE ] ) + hb_osnewline() )
         ErrorLevel( 1 )
         QUIT
      ENDIF

      IF s_aSAXEventCounts[ MXML_SAX_ELEMENT_OPEN ] != 20
         OutErr( sprintf( "MXML_SAX_ELEMENT_OPEN seen %d times, expected 20 times!",             ;
                               s_aSAXEventCounts[ MXML_SAX_ELEMENT_OPEN ] ) + hb_osnewline() )
         ErrorLevel( 1 )
         QUIT
      ENDIF
...
FUNCTION type_cb( hNode )                 /* O - Data type */
   ...
   DO CASE
      CASE cType == "integer" ;  RETURN MXML_INTEGER
      CASE cType == "opaque"  ;  RETURN MXML_OPAQUE
      CASE cType == "real"    ;  RETURN MXML_REAL
   ENDCASE
